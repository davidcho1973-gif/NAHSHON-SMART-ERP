...
{
    protected $signature = 'ops:digest {date? : YYYY-MM-DD, 기본=오늘} {--timezone= : Only sites in this timezone}';

    protected $description = '현장 상황실 하루 요약 발송';

    public function handle(OpsDigestService $service): int
    {
        $date = $this->argument('date') ? Carbon::parse($this->argument('date')) : Carbon::today($this->option('timezone') ?: config('app.timezone'));
        $r = $service->dispatchDigest($date, $this->option('timezone'));
