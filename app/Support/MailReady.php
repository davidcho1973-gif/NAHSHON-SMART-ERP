...
final class MailReady
{
    ...
    /**
     * 이 메일러로 실을 수 있는 <b>첨부 총량</b>. 상한을 정하는 곳은 여기 하나다.
     *
     * 상한이 여러 벌이면 반드시 어긋난다. 실제로 그랬다 — 일일 보고와 제출물 전달은
     * 각자 18MB 를 하드코딩해 두고 있었는데, Microsoft 365(Graph)로 보내는 순간
     * 전송 계층이 3MB 에서 막았다. 셋이 서로를 몰라서, 사진 붙은 저녁 마감보고서는
     * <b>켜는 날부터 매일</b> 실패하고 첨부 없는 아침 계획서만 나가는 상태가 됐다.
     * "메일은 되는데 저녁 것만 안 온다" 로 보여서 원인을 찾기도 어렵다.
     *
     * Graph 가 유난히 작은 이유 — <b>첨부가 base64 를 두 번 거친다.</b>
     * 처음 잡았던 2MB 가 여전히 실패해서 실제로 봉투를 만들어 재 봤더니 이랬다:
     *
     *   첨부 원본 2.0MB → MIME 2.7MB (첨부가 MIME 안에서 base64, ×1.37)
     *                   → Graph 요청 본문 3.6MB (MIME 전체를 다시 base64, ×1.33)
     *
     * 합쳐서 약 1.8배다. 그래서 3MB 상한에 넣으려면 첨부는 1.6MB 를 넘을 수 없고,
     * 본문·헤더 몫까지 빼면 1.4MB 가 안전선이다. 계산이 아니라 재 본 값이다.
     */
    public static function attachmentBudget(): int
    {
        $configured = (int) config('mail.max_attachment_bytes', 0);
        if ($configured > 0) {
            return $configured;
        }

        // Graph: 3MB ÷ 1.82(이중 base64) − 본문·헤더 몫.
        return (string) config('mail.default', 'log') === 'graph'
            ? 1_400_000
            : 18 * 1024 * 1024;
    }
