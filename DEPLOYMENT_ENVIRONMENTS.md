...
2. GitHub → Settings → Secrets and variables → Actions → **Secrets** 탭
   → **그 환경의 시크릿** 연필 아이콘 → 새 값으로 덮어쓰기.
   (나손은 `LARAVEL_CLOUD_DEPLOY_HOOK_NAHSHON`, DASOL 은 `..._PRODUCTION`,
   KSR 은 `..._STAGING`. 이름이 환경과 어긋나 있으니 아래 표를 보고 고른다.)
3. 훅이 통하는 것을 확인한 뒤(그 환경의 Deploy 잡이 훅 302 경고 없이 초록),
   원하면 `Push to deploy` 를 다시 꺼서 "테스트 통과 후에만 배포" 로 되돌린다.

**이 처방은 실제로 통했다 (2026-09-05 확인).** 나손 훅을 재생성해 시크릿을 덮어쓴 뒤
Actions 재실행에서 이렇게 나왔다 — 추측이 아니라 로그다.

    POST + commit_hash → HTTP 201
    1회차 — 서버: a53dcd8 / 기대: 4f49618
    2회차 — 서버: 4f49618 / 기대: 4f49618

즉 **훅 호출 → 배포 시작 → 서버가 그 커밋으로 바뀜**까지 한 줄로 이어지는 것이
로그로 확인됐다. 걸린 시간은 훅을 부른 뒤 서버가 바뀌기까지 약 1분이다.
...
**끝까지 도는 것을 로그로 확인했다 (2026-09-05).** 훅이 `HTTP 201` 을 주고, 확인이
1회차에 옛 커밋(`a53dcd8`)을, 2회차에 새 커밋(`4f49618`)을 읽었다. 훅을 부른 뒤 서버가
바뀌기까지 약 1분이다. 같은 잡에서 스케줄러 맥박도 5분 전으로 살아 있었다.
...
**위 표는 훅이 없을 때의 뒷길이다.** 훅이 있으면 잡은 배포를 걸고 서버가 바뀔 때까지
확인하며, 안 바뀌면 빨개진다. 훅 시크릿이 비어 올 때만 «읽어서 적기» 로 물러서는데,
그때는 잡이 초록으로 끝난다 — 내가 일으키지도 않은 배포가 늦다고 빨간 X 를 놓으면
사람은 곧 그 X 를 무시하는 법을 배우고, 그러면 진짜 실패도 함께 묻히기 때문이다.
...
>
> 그 옆의 재발급 버튼은 **잘 돌고 있을 때는** 누르지 말 것 — 주소가 바뀌어 시크릿을
> 다시 넣어야 한다. 다만 **이미 302 로 튕기고 있다면 재발급이 먼저다.** 그 값은 어차피
> 안 통하므로 잃을 것이 없고, 옛 값과 헷갈릴 일도 없어진다(위 「훅이 302 로 튕길 때」).

넣고 나면 해당 브랜치에 푸시할 때마다 이렇게 돈다.

    푸시 → 전체 테스트(2026-09-05 기준 1,713개) → 통과하면 Deploy Hook 호출 →
    {URL}/build-version 이 그 커밋으로 바뀔 때까지 최대 10분 확인 → Actions 화면에 결과 표시
