...
<div class="panel" style="max-width:500px;margin:auto"><h1>{{ $title }}</h1><p>{{ $note }}</p><div class="steps"><strong>카메라로 촬영</strong><span>→</span><strong>PIN 설정</strong><span>→</span><strong>출근 버튼</strong></div><img class="qr" src="{{ $qr }}" alt="{{ $title }} QR"><label for="link">원격 전달용 개인 링크</label><input id="link" readonly value="{{ $url }}"><button type="button" id="copy">링크 복사</button><p id="message" role="status"></p><small>이 QR은 직원 한 사람의 휴대폰 등록용입니다. 직원 본인이 휴대폰 기본 카메라로 스캔하며, 관리자는 PIN을 대신 정하지 않습니다.</small></div>
