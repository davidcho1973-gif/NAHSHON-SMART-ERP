...
const endpoints = {
    ...
    reanalyzeUntranslated: @json(route('document-intelligence.reanalyze-untranslated')),
...
// 영문으로만 남은 옛 분석 결과에 한국어를 붙인다. 건당 비용이 드는 바깥 호출이라
// 서버가 한 번에 보내는 수를 막아 두었고, 남은 건수를 알려 주면 사람이 나눠 누른다.
async function addKorean(){
    const btn=document.getElementById('ko-btn');if(!btn)return;
    btn.disabled=true;const old=btn.textContent;btn.textContent='보내는 중...';
    try{const d=await jsonFetch(endpoints.reanalyzeUntranslated,{method:'POST'});toast(d.message);loadDocuments()}
    catch(e){toast(e.message,true)}
    finally{btn.disabled=false;btn.textContent=old}
}
// ── 올리기 ─────────────────────────────────────────────────────────────
