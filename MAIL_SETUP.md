...
## 켜기 전에 — 자동 발송이 함께 살아납니다

메일이 켜지는 순간 **다음 날 08:30 작업계획서 · 18:30 마감보고서가 등록된 수신처로 진짜 나갑니다**
(`routes/console.php:69-70`).

그래서 켜기 전에 **[일일 보고] → [수신처 관리]** 를 먼저 열어 목록을 확인하세요.
**첫 주는 사내 주소만 남겨 두시길 권합니다.** "시험해 보다가 원청에 미완성 보고서가 날아갔다"
는 되돌릴 수 없습니다.

첫 실전 메일은 빈 테스트가 아니라 실제 업무 메일로 보내고, 받는 분께 **전화나 문자로
"받으셨습니까"를 직접 확인**하십시오. 대기업 메일 게이트웨이는 스팸으로 격리해도
보낸 쪽에 아무 통보를 하지 않습니다 — 우리 원장은 «발송» 초록불인데 상대는 못 받은 상태가
가장 위험하고, 첫 통만큼은 사람 확인이 유일한 증거입니다.
...
> **DNS 를 고칠 곳은 GoDaddy 가 아닙니다.**
> 네임서버는 `dns1~4.p03.nsone.net`(NS1)이고 SOA 관리자는 `domains+netlify.netlify.com` —
> 즉 **Netlify 가 이 도메인의 DNS 를 관리**하고 있습니다(웹사이트가 거기 있기 때문으로 보입니다).
> 2단계를 시작하기 전에 **Netlify 계정 접근**이 되는지 먼저 확인해 주세요.
> 오늘 하는 Graph 설정은 DNS 를 안 건드리므로 이 문제와 무관합니다.
